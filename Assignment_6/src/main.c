...
int main(void)
{
    ...
    //input pins
    PyHal_GPIO_SetDir(1,0);  // p
    PyHal_GPIO_SetDir(5,0);  // q
    PyHal_GPIO_SetDir(10,0);  // r

    	
    int p, q, r, out;	
    while(1)
    {
        p = PyHal_GPIO_Get(1); //taking input p
        q = PyHal_GPIO_Get(5);	//taking input q
        r = PyHal_GPIO_Get(10);	//taking input r
